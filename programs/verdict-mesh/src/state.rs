...
impl Dispute {
    /// Коло розгляду: 0 — початкова панель, 1 — розширена. Виводиться з
    /// `escalated`, а не зберігається окремо, бо ескалація буває рівно одна
    /// (`FR-027a`) — окреме поле було б другим джерелом того самого факту.
    ///
    /// Число потрібне тому, що відбиток голосу належить своєму колу: поданий у
    /// першому й не розкритий до підрахунку, він уже спізнився, і розкривати
    /// його в другому означало б дати присяжному вибирати, оприлюднювати свій
    /// голос чи ні, **побачивши перший підрахунок**.
